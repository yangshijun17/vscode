...
BTree creatTree(BTree root, int n)
{
    ...
    root->lchild=NULL;
    root->height=0;
    root->data.key=abs(rand())%1000;
    int term[MAX_NODE]={0};
    int flag;
    term[0]=root->data.key;
    for(int i=1;i<n;i++)
    {
        flag=0;
    ...
    while(n!=deep+1)
    {
        ...
        if(k==pow(2,n)-1)
        {
            ...
            for(int i=pow(2,n-1)-1;i<k;i++)
            {
                leftLineNum=0;
                rightLineNum=0;
                absotiveDistance[0]=findNum(array, term[i]->data.key, count(root, 0))*MAX_DIGITS;
                if(absotiveDistance[0]==-MAX_DIGITS)
                    continue;
                relativeDistance=absotiveDistance[0]-absotiveDistance[1];

                absotiveDistance[0]+=digits(term[i]->data.key)+2;

                absotiveDistance[1]=absotiveDistance[0];
                if(term[i]->lchild!=N&&term[i]->lchild!=NULL)
                {
                    leftLineNum=(absotiveDistance[0]-findNum(array, term[i]->lchild->data.key, count(root, 0))*MAX_DIGITS)-digits(term[i]->data.key)-4;
                    vertiLineArray[vertiLineNum++]=findNum(array, term[i]->lchild->data.key, count(root, 0))*MAX_DIGITS+2;
                }
                if(term[i]->rchild!=N&&term[i]->rchild!=NULL)
                {
                    rightLineNum=(findNum(array, term[i]->rchild->data.key, count(root, 0))*MAX_DIGITS-absotiveDistance[0])+2;
                    vertiLineArray[vertiLineNum++]=findNum(array, term[i]->rchild->data.key, count(root, 0))*MAX_DIGITS+1;
                }
